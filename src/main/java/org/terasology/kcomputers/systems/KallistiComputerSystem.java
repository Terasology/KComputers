...
/**
 * This system handles computer initialization, as well as - currently - entity propagation for
 * KallistiGatherConnectedEntitiesEvent.
 *
...
public class KallistiComputerSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    private final Set<EntityRef> computers = new HashSet<>();
    @In
    ...
    public void computerToggle(KallistiChangeComputerStateEvent event, EntityRef target) {
        ...
        try {
            ...
            if (instigator.exists() && instigator.hasComponent(ClientComponent.class)) {
                instigator.send(new ChatMessageEvent(s + ": " + (e.getMessage() != null ? e.getMessage() :
                        e.getClass().getName()), event.getMachine()));
    ...
    @ReceiveEvent
    public void computerDeactivated(BeforeDeactivateComponent event, EntityRef ref, BlockComponent blockComponent,
    ...
    @ReceiveEvent
    public void addConnectedEntitiesConnectable(KallistiGatherConnectedEntitiesEvent event, EntityRef ref,
    ...
    @ReceiveEvent
    public void addConnectedEntitiesInventory(KallistiGatherConnectedEntitiesEvent event, EntityRef entity,
    ...
    private void init(EntityRef ref, boolean force) throws Exception {
        ...
        MultiValueMap<Vector3i, TerasologyEntityContext> contextsPerPos =
                new CollectionBackedMultiValueMap<>(new HashMap<>(), ArrayList::new);

...
public class KallistiDisplayAuthoritySystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    private final MultiValueMap<EntityRef, EntityRef> displayListeners =
            new CollectionBackedMultiValueMap<>(new HashMap<>(), HashSet::new);
    private final Map<EntityRef, Object> lastSource = new HashMap<>();
    ...
    public void update(float delta) {
        for (EntityRef machine : displayListeners.keys()) {
            KallistiDisplayComponent displayComponent =
                    machine.getComponent(KallistiDisplayCandidateComponent.class).getDisplay();
            ...
            if (displayComponent != null) {
                ...
                if (lastSourceObj != sourceObj) {
                    ...
                    if (sourceObj != null) {
                        KComputersUtil.synchronize(machine, displayComponent.getSource(), Synchronizable.Type.INITIAL
                                , displayListeners.values(machine));
                    }
                } else {
                    if (sourceObj != null) {
                        KComputersUtil.synchronize(machine, displayComponent.getSource(), Synchronizable.Type.DELTA,
                                displayListeners.values(machine));
                    }
                }
            }
